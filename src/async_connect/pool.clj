(ns async-connect.pool
  (:require [async-connect.client :refer [IConnection] :as client]
            [clojure.spec :as s]
...
(def ^:private pooled-connections (volatile! {}))
...
(defn connect
  "Connect to a `port` of a `host` using `bootstrap`, and return a IConnection object, but before making
          ...
          (if found
            ...
            (do
              ...
              (let [{:keys [:client/channel] :as new-conn}
                        (merge (->PooledConnection)
                              (client/connect bootstrap host port read-ch write-ch)
                              {:pool/host host
                               :pool/port port})]
...
(defn sample-connect
  []
  (let [bootstrap (client/make-bootstrap {})
